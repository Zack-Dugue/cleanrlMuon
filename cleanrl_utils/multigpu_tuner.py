"""
multi_gpu_tuner.py — DEBUG LOGGING EDITION (Fail-Fast Enhanced)

Same as before, but:
- If any worker raises an error, the main process logs it and exits.
- After pool cleanup, if no results exist, tuner stops immediately.
- The outer study loop catches any fatal exception, logs it to tuner.log, and sys.exit(1).
"""

from __future__ import annotations
import os, sys, time, math, glob, runpy, traceback, logging
    ...
    def tune(self, num_trials: int, num_seeds: int):
        ...
        def objective(trial: optuna.Trial):
            ...
            for seed in range(num_seeds):
                ...
                try:
                    ...
                    for pool, shard, gpu_id in zip(pools, shards, self.gpus):
                        ...
                        try:
                            ...
                            MAIN_LOGGER.error(f"[TRIAL {trial.number}] Worker error on GPU {gpu_id}:\n{traceback.format_exc()}")
                            raise
                finally:
                    for p in pools:
                        try: p.close()
                ...
                # ⛔ ERROR HANDLING ADDITION — fail fast if no results
                if not results:
        ...
        # ⛔ ERROR HANDLING ADDITION — log & exit on any fatal exception
        try:
